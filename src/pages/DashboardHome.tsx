...
type Props = {
  firstName: string;
  /**
   * Lift a project brief into the dedicated onboarding flow. App.Shell
   * stashes the seed and navigates to `#onboarding`, where the full
   * discovery → requirements → architecture pipeline lives. Home itself
   * never owns polling, tokens, or job state.
   */
  onStartOnboarding: (description: string) => void;
};

export default function DashboardHome({ firstName, onStartOnboarding }: Props) {
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (description.length > MAX_DESCRIPTION_LENGTH) {
      setError(
        `Please keep your description under ${MAX_DESCRIPTION_LENGTH} characters.`
      );
      return;
    }
    setError(null);
    onStartOnboarding(description);
  }, [brief, onStartOnboarding]);
  ...
  return (
    <>
      ...
      <section className="home-block">
        ...
        />
        {error ? (
          <p className="home-input-error" role="alert">
            {error}
          </p>
        ) : null}
      </section>
